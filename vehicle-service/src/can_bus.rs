...
impl CanBus {
    pub fn open(iface: &str, demo: bool) -> Result<Self, String> {
        ...
        eprintln!(
            "sigma-racer-wingman-vehicle: SocketCAN on {iface}{}",
    ...
    pub fn poll(&mut self, state: &mut VehicleState, logger: &mut Option<CanLogger>) {
        loop {
            match self.socket.read_frame() {
                ...
                Err(err) => {
                    eprintln!("sigma-racer-wingman-vehicle: CAN read: {err}");
                    break;
                }
            }
        }
...
fn demo_loop(iface: &str, stop: Arc<AtomicBool>) {
    let socket = match CanSocket::open(iface) {
        Ok(socket) => socket,
        Err(err) => {
            eprintln!("sigma-racer-wingman-vehicle: CAN demo open {iface}: {err}");
            return;
        }
    };
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        for (id, payload) in encode_sim_frames(&state) {
            let frame = match CanFrame::from_raw_id(id, &payload) {
                Some(frame) => frame,
                None => {
                    eprintln!("sigma-racer-wingman-vehicle: CAN demo encode 0x{id:03X}");
                    continue;
                }
            };
            if let Err(err) = socket.write_frame(&frame) {
                eprintln!("sigma-racer-wingman-vehicle: CAN demo write: {err}");
            }
        }
